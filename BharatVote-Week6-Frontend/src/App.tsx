...
function App() {
  ...
  // KYC (for non-admin voters) - Week 6: Store voterId from KYC
  ...
  // Only restore KYC status AFTER admin check completes (to avoid skipping KYC for non-admins)
  useEffect(() => {
    if (!account || !isAdminCheckComplete) return;
    
    const key = `bv_kyc_${account.toLowerCase()}`;
    const voterIdKey = `bv_voter_id_${account.toLowerCase()}`;
    const fromStorage = localStorage.getItem(key);
    const storedVoterId = localStorage.getItem(voterIdKey);
    
    console.log('DEBUG App: Checking localStorage for KYC status');
    console.log('DEBUG App: Account:', account);
    console.log('DEBUG App: isAdmin:', isAdmin);
    console.log('DEBUG App: KYC in storage:', fromStorage);
    
    // Only restore KYC if user is NOT admin (admins don't need KYC)
    if (!isAdmin && fromStorage === "1") {
      console.log('DEBUG App: Restoring KYC status from localStorage');
      setIsKycVerified(true);
      if (storedVoterId) {
        setVoterIdKyc(storedVoterId);
      }
    } else if (!isAdmin) {
      console.log('DEBUG App: No KYC found in storage, will show KYC page');
      setIsKycVerified(false);
    }
  }, [account, isAdminCheckComplete, isAdmin]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log("[Probe] bytecode length:", code.length);
            ...
            <div className="max-w-2xl mx-auto">
              <div className="card p-8">
                <h1 className="text-3xl font-bold mb-6 text-center">
                  BharatVote - Week 6: Admin Dashboard & Deployment Automation
                </h1>
  ...
  // Only check KYC after admin check is complete
  if (isAdminCheckComplete && !isAdmin && !isKycVerified) {
    console.log('DEBUG App: Showing KYC page - isAdmin:', isAdmin, 'isKycVerified:', isKycVerified, 'isAdminCheckComplete:', isAdminCheckComplete);
    return (
      <I18nProvider>
        <KycPage
          account={account!}
